...
TITULO = "Documentación viva + asistente"
LEMA = "La documentación se escribe sola leyendo los workflows"
FRASE = ("Cuando el flujo cambia, la ficha se regenera y vuelve a decir la verdad. No puede quedarse desactualizada sin que se note.")
...
def medidas() -> dict:
    """Lo que hay en data/, fichas/ y los workflows del portafolio."""
    import csv
    from collections import Counter
    bandeja = []
    f = RAIZ / "data" / "bandeja_clasificada.csv"
    if f.exists():
        with f.open(encoding="utf-8", newline="") as fh:
            bandeja = list(csv.DictReader(fh))
    cat = Counter(c.get("categoria", "") for c in bandeja)
    humano = sum(1 for c in bandeja
                 if str(c.get("requiere_humano", "")).strip().lower()
                 in ("1", "true", "sí", "si", "yes"))
    fichas = sorted((RAIZ / "fichas").glob("ficha_*.md")) \
        if (RAIZ / "fichas").exists() else []
    secciones = 0
    if fichas:
...
def main() -> int:
    ...
    PASOS = [
        ("01", "documento", "Lee", "El JSON que n8n ya guarda",
         f"{d['nodos']}", f"nodos leídos"),
        ("02", "cpu", "Deriva", "La ficha de cada flujo",
         f"{d['derivadas']}/{d['secciones']}", f"secciones sin escribir"),
        ("03", "correo", "Clasifica", "El correo que llega",
         f"{d['correos']}", f"en {d['categorias']} categorías"),
        ("04", "alerta", "Frena", "Lo que exige criterio",
         f"{d['humano']}", f"van a una persona"),
    ]
    ...
    for i, (et, val) in enumerate((("Pruebas", f"47 pasando"), ("Fichas autogeneradas", f"{d['fichas']}"), ("Sin la IA", f"funciona con reglas"), ("Escrito a mano", f"2 secciones"))):
        x = m + i * ((W - 2 * m) / 4)
